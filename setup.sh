...
PYTHON_BIN="python3"
if ! "$PYTHON_BIN" -c 'import sys; sys.exit(sys.version_info.major != 3)' >/dev/null 2>&1; then
    PYTHON_BIN="python"
    if ! "$PYTHON_BIN" -c 'import sys; sys.exit(sys.version_info.major != 3)' >/dev/null 2>&1; then
        echo "Required: Python 3 is not available as python3 or python."
        exit 1
    fi
fi
check_command git
...
echo -e "${CYAN}Build an LLM Wiki with Codex and Jev${NC}"
echo ""
...
# ----- Step 5: Memory path -----
echo -e "${BOLD}Optional directory for non-secret L1 rule notes?${NC}"
echo -e "(For example: ~/.codex/wiki-memory/my-project/; Codex reads these explicitly.)"
read -p "Path [skip]: " memory_path
memory_path="${memory_path/#\~/$HOME}"
if [ "$memory_path" = "skip" ]; then memory_path=""; fi
echo ""
...
if [ -d "$wiki_path/.git" ]; then
    "$PYTHON_BIN" - "$wiki_path/.gitignore" << 'PYEOF'
import sys
from pathlib import Path
path = Path(sys.argv[1])
text = path.read_text(encoding="utf-8") if path.exists() else ""
rules = [".wiki-ingest/", ".env", ".env.*", "__pycache__/"]
...
"$PYTHON_BIN" - "$TOOL" "$pages_path" "$wiki_path" "$TEMPLATE_DIR" "$NAMESPACES" "$TODAY" << 'PYEOF'
import os
import sys

tool, pages_path, wiki_path, template_dir, namespace_text, today = sys.argv[1:]
namespaces = namespace_text.split()
...
write_config() {
    "$PYTHON_BIN" - "$CONFIG_FILE" "$TOOL" "$wiki_path" "$PAGES_DIR" "$memory_path" "$NAMESPACES" << 'PYEOF'
import json
import sys
from datetime import date
from pathlib import Path

path, tool, wiki, pages, memory, namespaces = sys.argv[1:]
values = {"tool": tool, "wiki_path": str(Path(wiki).resolve()), "pages_dir": pages,
          "memory_path": memory, "l1_verify_days": 90}
lines = ["# llm-wiki configuration", f"# Generated by setup.sh on {date.today()}"]
lines += [f"{key}: {json.dumps(value)}" for key, value in values.items()]
lines += ["namespaces:"] + [f"  - {json.dumps(ns)}" for ns in namespaces.split()]
lines += ["ingest:", "  mode: jev", "  artifact_dir: .wiki-ingest", "  jev:",
          "    model: jev-1.13.0", "    live: false", "    max_requests: 1000",
          '    "yes": 0.8', '    "no": 0.2', "    confidence: 0.8"]
...
echo ""
echo -e "${BOLD}Install the wiki skill for Codex?${NC}"
echo 'This installs .agents/skills/wiki and adds a pointer to project AGENTS.md.'
read -p "Project path (or 'skip'): " project_path

if [ "$project_path" != "skip" ] && [ -n "$project_path" ]; then
    project_path="${project_path/#\~/$HOME}"
    "$PYTHON_BIN" "$SCRIPT_DIR/scripts/install_skill.py" --project "$project_path" --config "$CONFIG_FILE"
fi
...
echo -e "  2. In Codex, try: ${CYAN}\$wiki ingest \"your first source\"${NC}"
echo -e "  3. Run ${CYAN}\$wiki status${NC} to see your wiki metrics"
echo '  Jev starts offline. Set TYPESAFE_API_KEY and ingest.jev.live: true for live judgments.'
echo ""
